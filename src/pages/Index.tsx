...
<section className="mb-5">
  <div className="flex justify-between items-center mb-3">
    <h2 className={cn("text-xl font-medium", isMobile && "text-base")}>Leagues</h2>
        ...
        {tournamentsLoading ? (
          [...Array(4)].map((_, i) => (
            <div key={i} className="snap-center flex-shrink-0">
              <div className="w-14 h-14 rounded-full bg-gray-700 flex items-center justify-center overflow-hidden animate-pulse"></div>
            </div>
          ))
        ) : (
      ...
      <div className="flex flex-wrap gap-6 justify-start">
        {tournamentsLoading ? (
          ...
          tournaments?.map((tournament) => {
            ...
            console.log("Tournament Logo URL:", tournament.logo_url); // Debugging Log

            return (
              <TournamentCard 
                key={tournament.id}
                gradientClass={tournament.gradient_class || 'bg-gradient-to-r from-blue-500 to-purple-500'}
                date={tournament.date}
                logoUrl={tournament.logo_url}  // ✅ Added this line
              />
            );
          })
        )}
  ...
  {isMobile ? (
      ...
      <div className="flex space-x-4 snap-x snap-mandatory">
        {tournamentsLoading ? (
          ...
          tournaments?.slice(0, 4).map((tournament, index) => {
            const imagePath = `/c${(index % 4) + 1}.png`; // Assigns images c1.png to c4.png
            return (
              <div key={tournament.id} className="snap-center flex-shrink-0">
                <TournamentCard 
                  gradientClass="bg-white"
                  name={tournament.name}  // ✅ Pass tournament name
                  date={tournament.date}  // ✅ Pass tournament date
                  logoUrl={imagePath}     // ✅ Pass tournament logo
                />
              </div>
            );
          })
    ...
    <div className="bg-black rounded-xl p-4 md:p-8">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 justify-items-center">
        {tournamentsLoading ? (
          [...Array(4)].map((_, i) => (
            <div key={i} className="w-32 h-32 bg-black rounded-xl flex flex-col items-center justify-center animate-pulse"></div>
          ))
        ) : (
          tournaments?.slice(0, 4).map((tournament, index) => {
            const imagePath = `/c${(index % 4) + 1}.png`; // Assigns images c1.png to c4.png
